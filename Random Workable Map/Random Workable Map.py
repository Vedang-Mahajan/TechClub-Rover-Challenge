...
# Instances for more than 1 open cell for a certain cell
instances = []
instanceCells = {}
...
def createMap():
    # Append random open/closed blocks to arena_map
    for i in range(8):
        row_list = []
        for j in range(8):
            row_list.append(random.randint(0, 1))
...
while init_x != target_x and init_y != target_y and mapFound == False:
    ...
    if len(openCell) == 1:
        ...
        instances.append(
            [(str(init_x) + " " + str(init_y)), (len(openCell) - 1)])
        a = openCell[0]
        if ((str(a[0]) + " " + str(a[1]))) not in map_cells:
            map_cells[str(a[0]) + " " + str(a[1])] = 0
            init_x = a[0]
            ...
            for a in openCell:
                if ((str(a[0]) + " " + str(a[1]))) not in map_cells:
                    ...
                    init_x = a[0]
                    init_y = a[1]
        instanceCells[(str(init_x) + " " + str(init_y))] = ()
    else:
        if len(instances) != 0:
            raw_instance = instances[(len(instances) - 1)][0]
            x_y_list = raw_instance.split(" ")
            init_x = x_y_list[0]
            init_y = x_y_list[1]
        else:
            pass
